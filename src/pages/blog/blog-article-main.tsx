...
import { getBlog, blogs } from "@/data/catalog";

const BlogArticleMain = ({ slug }: { slug: string }) => {
          ...
          <article className="mr-article">
            <div className="container container-1000">
              <div className="mr-article-head">
                <span className="mr-article-cat">{blog.category}</span>
                <h1 className="mr-article-title">{blog.title}</h1>
                <div className="mr-article-meta"><span>By {blog.author}</span><em>•</em><span>{blog.date}</span><em>•</em><span>{blog.readTime}</span></div>
              </div>
              <div className="mr-article-hero"><SmartImage src={blog.image} alt={blog.title} label={blog.category} ratio="16 / 9" /></div>
              <div className="mr-article-body">
                <p className="mr-article-lead">{blog.excerpt}</p>
                ...
                <div className="mr-grid mr-grid-3">
                  {more.map((b) => (
                    <article className="mr-blog-card" key={b.id}>
                      ...
                      <div className="mr-blog-card-body"><span className="mr-blog-card-cat">{b.category}</span><h3 className="mr-blog-card-title"><Link href={`/blog-details/${b.slug}`}>{b.title}</Link></h3><Link href={`/blog-details/${b.slug}`} className="mr-blog-card-link">Read Article →</Link></div>
                    </article>
                  ))}
                </div>
              </div>
