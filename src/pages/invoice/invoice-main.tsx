...
import type { BackendOrder, BackendAddressSnapshot } from "@/types/backend";
...
function sameAddress(a: BackendAddressSnapshot, b: BackendAddressSnapshot) {
...
function InvoiceSheet({ order }: { order: BackendOrder }) {
      ...
      <div className="mr-invoice-summary">
        <div className="mr-invoice-summary-box">
          ...
          {order.gstDetails.isInterState ? (
            <div className="mr-invoice-srow"><span>IGST</span><span>{formatINR(order.pricing.igst)}</span></div>
          ) : (
            <>
