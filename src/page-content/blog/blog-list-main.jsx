...
const BlogListMain = ({ blogs, page, totalPages, totalResults, category, tag, search }) => {
      ...
      <main>
        ...
        <section className="mr-blog">
          <div className="container container-1400">
            ...
            {hasFilters && (<div style={{ marginBottom: 30 }}>
                <Link href="/blog" className="mr-shop-clear">Clear filters</Link>
              </div>)}

            {blogs.length === 0 ? (<div className="mr-shop-empty">
                ...
                <div className="mr-grid mr-grid-3">
                  {blogs.map((b) => (<article className="mr-blog-card" key={b.id}>
                ...
                {totalPages > 1 && (<div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 16, marginTop: 50 }}>
                    <Link href={buildHref(page - 1, { category, tag, search })} className="mr-btn-outline" aria-disabled={page <= 1} style={page <= 1 ? { pointerEvents: "none", opacity: 0.4 } : undefined}>
                      ← Previous
                    </Link>
                    <span style={{ fontSize: 14 }}>Page {page} of {totalPages}</span>
                    <Link href={buildHref(page + 1, { category, tag, search })} className="mr-btn-outline" aria-disabled={page >= totalPages} style={page >= totalPages ? { pointerEvents: "none", opacity: 0.4 } : undefined}>
                      Next →
                    </Link>
                  </div>)}
              </>)}
