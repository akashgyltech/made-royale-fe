...
export const products = [
    {
        ...
        upsellIds: ['p-benares-rug', 'p-lucknow-tv-unit'], relatedIds: ['p-travancore-recliner'],
    },
    {
        ...
        upsellIds: ['p-mysore-floor-lamp', 'p-jaipur-side-table'], relatedIds: ['p-jodhpur-lsofa'],
    },
    {
        ...
        upsellIds: ['p-jaipur-side-table'], relatedIds: ['p-kashmir-wardrobe'],
    },
    {
        ...
        upsellIds: ['p-benares-rug'], relatedIds: ['p-lucknow-tv-unit'],
    },
    {
        ...
        warranty: '5 Year Warranty', assembly: 'Free expert installation',
        upsellIds: ['p-jaipur-side-table'], relatedIds: ['p-awadh-storage-bed'],
    },
    {
        ...
        upsellIds: ['p-jaipur-side-table', 'p-mysore-floor-lamp'], relatedIds: ['p-travancore-recliner'],
    },
    {
        ...
        upsellIds: ['p-mysore-floor-lamp'], relatedIds: ['p-hyderabad-accent-chair'],
    },
    {
        ...
        warranty: '3 Year Warranty', assembly: 'Free expert installation',
        upsellIds: ['p-benares-rug'], relatedIds: ['p-shimla-bookshelf'],
    },
    {
        ...
        upsellIds: ['p-jodhpur-lsofa'], relatedIds: ['p-mysore-floor-lamp'],
    },
    {
    ...
    },
    {
        ...
        warranty: '3 Year Warranty', assembly: 'Free expert installation',
        upsellIds: ['p-benares-rug'], relatedIds: ['p-jodhpur-lsofa'],
    },
    {
...
export const blogs = [
    ...
    { id: 'b-2', slug: 'styling-the-royal-living-room', title: 'How to Style a Royal Yet Liveable Living Room', excerpt: 'Layering velvet, brass and marble without tipping into excess — a designer’s guide to understated grandeur.', category: 'Interior Styling', author: 'Ishita Kapoor', date: '28 May 2026', readTime: '5 min read', image: '', content: ['Royal does not mean crowded. The secret to grandeur is restraint — one hero piece, supported by quieter companions.', 'Anchor the room with a statement sofa such as the Jodhpur L-Shaped Modular, then let a marble-and-brass coffee table and a hand-knotted rug do the supporting work.', 'Keep your palette to three tones: a jewel colour, a warm neutral, and a metallic accent. Repeat them across the room for a cohesive, curated feel.'] },
...
export function validateCoupon(code, orderAmount) {
    ...
    if (orderAmount < coupon.minOrder)
        return { valid: false, discount: 0, message: `Minimum order of ${formatINR(coupon.minOrder)} required` };
...
export function getRelated(product) {
    ...
    const explicit = ids.map((id) => getProductById(id)).filter((p) => !!p);
...
export function getUpsell(product) {
    return (product.upsellIds || []).map((id) => getProductById(id)).filter((p) => !!p).slice(0, 4);
}
export function sortProducts(list, sort = 'featured') {
    ...
    switch (sort) {
        ...
        case 'rating':
            ...
            break;
        case 'newest':
            out.sort((a, b) => (b.badge === 'New Arrival' ? 1 : 0) - (a.badge === 'New Arrival' ? 1 : 0));
            break;
...
export function filterProducts(filter) {
    ...
    if (filter.sub)
        list = list.filter((p) => p.subcategorySlug === filter.sub);
...
export function emiPerMonth(price, months = 12) { return Math.round(price / months); }
// Care instructions derived from a product's material
export function careFor(material) {
    const m = material.toLowerCase();
    const care = [];
    if (/wood|teak|sheesham|walnut|mango|rosewood/.test(m))
        care.push('Dust weekly with a soft, dry cloth', 'Apply a wood conditioner twice a year', 'Keep away from direct sunlight & AC vents');
    if (/velvet|linen|fabric|boucle/.test(m))
        care.push('Vacuum the upholstery gently every fortnight', 'Blot spills immediately — never rub');
    if (/leather/.test(m))
        care.push('Wipe with a dry cloth; condition twice a year', 'Avoid direct heat & sharp objects');
    if (/marble/.test(m))
        care.push('Reseal the marble surface annually', 'Use coasters; wipe spills promptly to avoid stains');
    if (/brass|metal/.test(m))
        care.push('Polish brass gently with a dry microfibre cloth');
    if (/silk|wool/.test(m))
        care.push('Professional dry-clean only', 'Rotate periodically for even wear');
    if (!care.length)
        care.push('Dust regularly with a soft, dry cloth', 'Avoid harsh chemical cleaners');
    return care;
}
export function boxContentsFor(name) {
