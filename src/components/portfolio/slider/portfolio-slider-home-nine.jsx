...
// bg img
function BgImg({ imgSrc, id }) {
    return (<Image src={imgSrc} alt="bg-img" className={`tp-porfolio-10-bg-${id}`} style={{ height: "auto" }}/>);
}
export default function PortfolioSliderHomeNine({ setWhiteCls }) {
    const [activeBg, setActiveBg] = React.useState('tp-porfolio-10-bg-4');
    function handleActiveBg(id) {
    ...
    return (<div className="tp-porfolio-10-area tp-porfolio-10-height p-relative">
        ...
        <div className="row">
          <div className="col-xl-12">
            <div className="tp-porfolio-10-title-wrap z-index-5">
              <ul>
                {portfolio_titles.map((item) => (<li key={item.id} onMouseEnter={() => handleActiveBg(item.id)} onMouseLeave={() => handleDeactiveBg()} className={activeBg === `tp-porfolio-10-bg-${item.id}` ||
                activeBg === null
                ? "active"
