...
export default function BlogClassicArea() {
    ...
    return (<section id="postbox" className="postbox__area tp-blog-sidebar-sticky-area pt-120 pb-80">
      <div className="container">
        <div className="row">
          <div className="col-xxl-8 col-xl-8 col-lg-8">
            <div className="postbox__wrapper">
              {currentItems.map((item) => (<article key={item.id} className="postbox__item mb-80">
                  {!item.blogQuote && !item.blogQuoteTwo && !item.imgSlider && (<div className="postbox__thumb">
                      <Link href={`/blog-details/${item.id}`}>
                        <Image src={item.img} alt="blog-img"/>
                      </Link>
                      {item.video && (<div className="postbox__play-btn">
                          <a onClick={() => playVideo("rVHxkxJM3rY")} className="popup-video pointer">
                            <i className="fa-sharp fa-solid fa-play"></i>
                          </a>
                        </div>)}
                    </div>)}
                  {item.imgSlider && (<div className="postbox__thumb w-img">
                      <div className="postbox__thumb-slider p-relative">
                        <Swiper {...slider_setting} modules={[Navigation, Pagination]} className="swiper-container postbox__thumb-slider-active fix">
                          {item.images &&
                    item.images.map((src, i) => (<SwiperSlide key={i}>
                                <Image src={src} alt=""/>
                              </SwiperSlide>))}
                        </Swiper>
                  ...
                  {!item.blogQuote && !item.blogQuoteTwo && (<div className="postbox__content">
                  ...
                  {item.blogQuoteTwo && (<div className="postbox__link-post-wrap d-flex align-items-center">
                  ...
                  {item.blogQuote && (<div className="postbox__blockquote">
                      <blockquote>
