...
import { legalDocList } from "@/data/legal";
const anchor = (h) => h.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
function extractHeadingsAndInjectIds(html) {
    const toc = [];
    const used = new Set();
    const withIds = html.replace(/<h([1-3])([^>]*)>([\s\S]*?)<\/h\1>/gi, (match, level, attrs, inner) => {
...
export default function LegalPage({ slug, doc, cms }) {
          ...
          <div className="container container-1300">
            <div className="mr-legal-layout">
              <aside className="mr-legal-toc">
                ...
                <nav>
                  {toc.map((t) => (<a key={t.id} href={`#${t.id}`}>{t.heading}</a>))}
                </nav>
              ...
              <div className="mr-legal-content">
                {cms.updatedAt && (<p className="mr-legal-updated">
                ...
                {relatedLinks.length > 0 && (<div className="mr-legal-related">
                    <span>Related policies</span>
                    <div className="mr-legal-related-links">
                      {relatedLinks.map((d) => (<Link key={d.slug} href={`/${d.slug}`}>{d.title}</Link>))}
                    </div>
                  </div>)}
              </div>
            </div>
    ...
    if (!doc)
        return null;
    return (<>
      <LuxBreadcrumb subtitle={doc.subtitle} title={doc.title} crumbs={[{ label: "Home", href: "/" }, { label: "Legal" }, { label: doc.title }]}/>
      <section className="mr-legal">
        <div className="container container-1300">
          <div className="mr-legal-layout">
            <aside className="mr-legal-toc">
            ...
            <div className="mr-legal-content">
              ...
              {doc.sections.map((s) => (<div key={s.heading} id={anchor(s.heading)} className="mr-legal-section">
                  <h2>{s.heading}</h2>
