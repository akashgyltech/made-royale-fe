...
function reviewerName(customer) {
    return typeof customer === 'string' ? 'Shizenta Customer' : customer.name || 'Shizenta Customer';
}
export default function ProductDetail({ product, initialReviews, related }) {
    ...
    return (<div className="mr-pdp">
      <div className="container container-1400">
        ...
        <div className="mr-pdp-top">
          ...
          <div className="mr-pdp-gallery">
            ...
            <div className="mr-pdp-thumbs">
              {gallery.map((g, i) => (<button key={i} className={`mr-pdp-thumb ${i === active ? 'is-active' : ''}`} onClick={() => setActive(i)} aria-label={`View ${i + 1}`}>
          ...
          <div className="mr-pdp-summary">
            ...
            {product.highlights.length > 0 && (<ul className="mr-pdp-highlights">
                {product.highlights.map((h) => <li key={h}><span className="mr-pdp-tick">✓</span>{h}</li>)}
              </ul>)}

            {product.colors.length > 0 && (<div className="mr-pdp-option">
                <span className="mr-pdp-option-label">Upholstery / Finish: <strong>{color}</strong></span>
                <div className="mr-swatches">
                  {product.colors.map((c) => (<button key={c.name} className={`mr-swatch ${color === c.name ? 'is-active' : ''}`} style={{ background: c.hex }} onClick={() => setColor(c.name)} aria-label={c.name} title={c.name}/>))}
                </div>
              </div>)}
        ...
        <div className="mr-pdp-tabs">
          ...
          <div className="mr-pdp-tabbody">
            {tab === 'description' && (<div className="mr-pdp-desc">
            ...
            {tab === 'specs' && (<table className="mr-pdp-spectable"><tbody>
            ...
            {tab === 'dimensions' && (<div className="mr-pdp-dims">
            ...
            {tab === 'reviews' && (<div className="mr-pdp-reviews">
                <div className="mr-pdp-review-top">
                  ...
                  <div className="mr-pdp-review-bars">
                    {[5, 4, 3, 2, 1].map((star, i) => (<div className="mr-pdp-review-bar" key={star}>
                        <span>{star}★</span>
                ...
                {reviews.length === 0 ? (<p className="mr-catproducts-empty">No reviews yet — be the first to share your experience.</p>) : (<div className="mr-pdp-review-list">
                    {reviews.map((r) => {
                    const name = reviewerName(r.customer);
                    return (<div key={r.id} className="mr-pdp-review">
                ...
                {hasMoreReviews && (<div className="text-center mt-30">
        ...
        {related.length > 0 && (<div className="mr-pdp-related">
      ...
      {showReview && (<ReviewModal productName={product.name} onClose={() => setShowReview(false)} onSubmit={submitReview}/>)}
...
function PincodeCheck({ toast }) {
    ...
    return (<div className="mr-pdp-pincode">
...
function ReviewModal({ productName, onClose, onSubmit }) {
    ...
    const submit = async (e) => {
        ...
        setErr('');
        if (!title.trim()) {
            setErr('Please add a short review title.');
            return;
        }
        if (comment.trim().length < 10) {
            setErr('Please write at least 10 characters in your review.');
            return;
        }
        setSubmitting(true);
        try {
    ...
    return (<div className="mr-review-overlay" onClick={(e) => { if (e.target === e.currentTarget)
        onClose(); }}>
        ...
        <form onSubmit={submit} noValidate>
          <div className="mr-review-field">
            ...
            <div className="mr-review-stars">
              {[1, 2, 3, 4, 5].map((s) => (<button type="button" key={s} className={`mr-review-star ${(hover || rating) >= s ? 'is-on' : ''}`} onMouseEnter={() => setHover(s)} onMouseLeave={() => setHover(0)} onClick={() => setRating(s)} aria-label={`${s} star`}>★</button>))}
