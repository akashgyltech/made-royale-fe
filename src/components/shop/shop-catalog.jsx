...
export default function ShopCatalog({ categories, products, total, totalPages, page, priceFrom, activeCategoryName }) {
    ...
    function pushParams(next) {
        ...
        Object.entries(next).forEach(([k, v]) => { if (v === null || v === '')
            sp.delete(k);
        else
            sp.set(k, v); });
    ...
    return (<div className="mr-shop">
      <div className="container container-1500">
        ...
        {activeCat && (<div className="mr-shop-subnav">
            <Link href={`/shop?category=${activeCat.slug}`} className={!sub ? 'is-active' : ''}>All {activeCat.name}</Link>
            {activeCat.subcategories.map((s) => (<Link key={s.id} href={`/shop?category=${activeCat.slug}&sub=${s.slug}`} className={sub === s.slug ? 'is-active' : ''}>{s.name}</Link>))}
          </div>)}
        ...
        <div className="mr-shop-layout">
          <aside className={`mr-shop-sidebar ${showFilters ? 'is-open' : ''}`}>
            ...
            <div className="mr-filter-block">
              ...
              <ul className="mr-filter-list">
                ...
                {categories.map((c) => (<li key={c.id}>
                    ...
                    {category === c.slug && c.subcategories.length > 0 && (<ul className="mr-filter-sublist">
                        {c.subcategories.map((s) => (<li key={s.id}><button className={sub === s.slug ? 'is-active' : ''} onClick={() => pushParams({ sub: sub === s.slug ? null : s.slug })}>{s.name}</button></li>))}
                      </ul>)}
                  </li>))}
              </ul>
            </div>
            ...
            <div className="mr-filter-block">
              ...
              <ul className="mr-filter-list">
                {PRICE_STEPS.map((p, i) => (<li key={i}><button className={String(i) === priceIdx ? 'is-active' : ''} onClick={() => pushParams({ price: String(i) === priceIdx ? null : String(i) })}>{p.label}</button></li>))}
              </ul>
            </div>
            ...
            <div className="mr-filter-block">
              ...
              <div className="mr-filter-chips">
                {COLLECTIONS.map((c) => (<button key={c} className={`mr-chip ${collection === c ? 'is-active' : ''}`} onClick={() => pushParams({ collection: collection === c ? null : c })}>{c}</button>))}
              </div>
            </div>
          ...
          <div className="mr-shop-main">
            {products.length === 0 ? (<div className="mr-shop-empty">
                ...
                {totalPages > 1 && (<Pagination pageCount={totalPages} forcePage={page - 1} handlePageClick={(e) => {
