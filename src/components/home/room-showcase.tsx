...
import { rooms, filterByRoom } from "@/data/catalog";
...
export default function RoomShowcase() {
  return (
    <section className="mr-collections" id="rooms">
      <div className="container container-1400">
        <SectionHeader subtitle="Shop by Room" title="Furnish Every Space" />
        <div className="mr-collections-grid">
          {rooms.map((r) => {
            const count = filterByRoom(r).length;
            return (
              <Link key={r.slug} href={`/room/${r.slug}`} className={`mr-collection-card${r.image ? " has-image" : ""}`}>
                {r.image && (
