...
function hash(s) { let h = 0; for (let i = 0; i < s.length; i++)
    h = (h << 5) - h + s.charCodeAt(i); return Math.abs(h); }
function monogram(name) {
    const w = name.replace(/[^a-zA-Z ]/g, '').trim().split(/\s+/);
    return (w.length === 1 ? w[0].slice(0, 2) : w[0][0] + w[1][0]).toUpperCase();
}
export default function SmartImage({ src, alt, ratio = '4 / 3', label, glyph, className = '', rounded = 0 }) {
    if (src) {
        return (<div className={`mr-img ${className}`} style={{ aspectRatio: ratio, borderRadius: rounded }}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={src} alt={alt} loading="lazy"/>
      </div>);
    }
    const [c1, c2] = GRADIENTS[hash(alt) % GRADIENTS.length];
    return (<div className={`mr-ph ${className}`} style={{ aspectRatio: ratio, borderRadius: rounded, background: `linear-gradient(145deg, ${c1} 0%, ${c2} 100%)` }} role="img" aria-label={alt}>
