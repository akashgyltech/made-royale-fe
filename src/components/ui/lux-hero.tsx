...
export interface HeroCrumb { label: string; href?: string; }
export interface HeroStat { value: string; label: string; }
export interface HeroAction { label: string; href: string; variant?: 'gold' | 'ghost' }
...
export default function LuxHero({
  banner, image, eyebrow, title, intro, crumbs, stats, actions,
}: {
  banner?: string;
  image?: string;
  eyebrow?: string;
        ...
        <div className="mr-cathero-inner">
          ...
          {actions && actions.length > 0 && (
            <div className="mr-cathero-actions">
              {actions.map((a, i) => (
                <Link key={i} href={a.href} className={a.variant === 'ghost' ? 'mr-btn-outline mr-cathero-ghost' : 'mr-btn-gold'}>
