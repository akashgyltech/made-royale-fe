...
const emptyAddress = { name: '', phone: '', pincode: '', line1: '', line2: '', city: '', state: '' };
function fromBackendAddress(a) {
...
export default function CheckoutFlow() {
    ...
    const addressComplete = useMemo(() => !!(address.name.trim() && /^\d{10}$/.test(address.phone) && address.line1.trim() && address.city.trim() && address.state && /^\d{6}$/.test(address.pincode)), [address]);
    ...
    useEffect(() => {
        if (!isLoggedIn || !addressComplete || itemsForApi.length === 0) {
            setQuote(null);
            setQuoteError(null);
            return;
        }
    ...
    const validateAddress = () => {
        ...
        if (!billingSame) {
            ...
            if (!/^\d{10}$/.test(billing.phone))
                e.bPhone = 'Enter a 10-digit mobile number';
            if (!/^\d{6}$/.test(billing.pincode))
                e.bPincode = 'Enter a 6-digit pincode';
            if (!billing.line1.trim())
                e.bLine1 = 'Required';
            if (!billing.city.trim())
                e.bCity = 'Required';
            if (!billing.state)
                e.bState = 'Select a state';
        }
        if (gstEnabled) {
            if (!/^[0-9A-Z]{15}$/.test(gst.gstin.toUpperCase()))
    ...
    const placeOrder = async () => {
        ...
        if (!quote) {
            toast('Please wait for your order total to finish calculating', 'error');
            return;
        }
        setPlacing(true);
        try {
    ...
    if (!isLoggedIn) {
        return (<div className="container container-1300">
    ...
    return (<div className="mr-checkout">
      <div className="container container-1300">
        ...
        <div className="mr-checkout-layout">
          <div className="mr-checkout-main">
            {step === 0 && (<div className="mr-checkout-panel">
                ...
                {savedAddresses.length > 0 && (<div className="mr-choice-list" style={{ marginBottom: 24 }}>
                    {savedAddresses.map((a) => (<label key={a.id} className={`mr-choice ${selectedSavedId === a.id ? 'is-active' : ''}`}>
                ...
                {!billingSame && (<div className="mr-form-grid mr-gst-grid">
                ...
                {gstEnabled && (<div className="mr-form-grid mr-gst-grid">
            ...
            {step === 1 && (<div className="mr-checkout-panel">
                ...
                {!quoteLoading && quote && quote.shippingMethods.length > 0 && (<div className="mr-choice-list">
                    {quote.shippingMethods.map((m) => (<label key={m.key} className={`mr-choice ${shippingMethod === m.key ? 'is-active' : ''}`}>
                ...
                {!quoteLoading && !quote && (<div className="mr-checkout-info">
                    <span>ℹ️</span>
            ...
            {step === 2 && (<div className="mr-checkout-panel">
                <h3 className="mr-checkout-panel-title">Payment Method</h3>
                <div className="mr-pay-grid">
                  {PAYMENT_OPTIONS.map((p) => (<button key={p.id} type="button" className={`mr-pay-method ${paymentMethod === p.id ? 'is-active' : ''}`} onClick={() => setPaymentMethod(p.id)}>
            ...
            {step === 3 && (<div className="mr-checkout-panel">
                ...
                <div className="mr-review-block">
                  ...
                  <div className="mr-review-items">
                    {(quote?.items ?? fallbackItems).map((it) => (<div className="mr-review-item" key={it.id}>
            ...
            <div className="mr-checkout-nav">
              ...
              {step < 3 ? (<button className="mr-btn-gold" onClick={() => void next()} disabled={placing}>Continue</button>) : (<button className="mr-btn-gold mr-place-btn" onClick={() => void placeOrder()} disabled={placing || !quote}>
          ...
          <aside className="mr-checkout-summary">
            ...
            <div className="mr-checkout-summary-items">
              {(quote?.items ?? fallbackItems).map((it) => (<div className="mr-checkout-summary-item" key={it.id}>
            ...
            <div className="mr-cart-coupon">
              <input value={couponInput} onChange={(e) => setCouponInput(e.target.value)} placeholder="Coupon code" onKeyDown={(e) => { if (e.key === 'Enter')
        applyCoupon(); }}/>
            ...
            {quote ? (<>
                <div className="mr-cart-summary-rows">
                  ...
                  {quote.isInterState ? (<div className="mr-cart-summary-row"><span>IGST</span><span>{formatINR(quote.igst)}</span></div>) : (<>
...
function Field({ label, error, children, full }) {
    return (<div className={`mr-field ${full ? 'mr-field-full' : ''} ${error ? 'has-error' : ''}`}>
