...
import type { BackendOrder, BackendAddress } from '@/types/backend';
      ...
      <div className="container container-1300">
        <div className="mr-account-layout">
          ...
          <div className="mr-account-content">
            {!isLoggedIn && (
              <div className="mr-account-banner"><div><strong>You’re browsing as a guest</strong><p>Sign in to view your orders, wishlist and addresses.</p></div><button className="mr-btn-gold" onClick={() => openAuthModal('login')}>Sign In</button></div>
            )}
            ...
            {tab === 'overview' && (
              <div className="mr-account-panel">
                ...
                {!isLoggedIn ? (
                  <EmptyState glyph="📦" title="Sign in to view your orders" text="Your order history will appear here." action={<button className="mr-btn-gold" onClick={() => openAuthModal('login')}>Sign In</button>} />
                ) : ordersLoading ? (
                  <p className="mr-cart-summary-tax">Loading…</p>
                ) : orders.length === 0 ? (
                  <EmptyState glyph="📦" title="No orders yet" text="Your future heirlooms will appear here." action={<Link href="/shop" className="mr-btn-gold">Start Shopping</Link>} />
            ...
            {tab === 'orders' && (
              <div className="mr-account-panel">
                ...
                {!isLoggedIn ? (
                  ...
                  <p className="mr-cart-summary-tax">Loading your orders…</p>
                ) : orders.length === 0 ? (
                  <EmptyState glyph="📦" title="No orders yet" text="Once you place an order, you can track it here." />
                ) : <div className="mr-account-orders">{orders.map((o) => <OrderRow key={o.id} order={o} />)}</div>}
              </div>
            )}
            ...
            {tab === 'wishlist' && (
              <div className="mr-account-panel">
                ...
                {!isLoggedIn ? (
                  <EmptyState glyph="♡" title="Sign in to view your wishlist" text="Save pieces you love to find them here." action={<button className="mr-btn-gold" onClick={() => openAuthModal('login')}>Sign In</button>} />
                ) : wishProducts.length === 0 ? (
                  <EmptyState glyph="♡" title="Your wishlist is empty" text="Save pieces you love to find them here." />
                ) : <div className="mr-grid mr-grid-3">{wishProducts.map((p) => <ShopItem key={p.id} product={p} />)}</div>}
              </div>
            )}
...
function OrderRow({ order }: { order: BackendOrder }) {
  return (
...
interface AddressFormState { label: NonNullable<BackendAddress['label']>; name: string; phone: string; line1: string; line2: string; city: string; state: string; pincode: string; }
const emptyAddressForm: AddressFormState = { label: 'home', name: '', phone: '', line1: '', line2: '', city: '', state: '', pincode: '' };
...
function AddressBook() {
  ...
  const load = useCallback(async () => {
    if (!isLoggedIn) { setAddresses([]); return; }
    setLoading(true);
    try { setAddresses(await profileApi.getAddresses()); } catch { /* ignore */ } finally { setLoading(false); }
  }, [isLoggedIn]);
  ...
  const startEdit = (a: BackendAddress) => {
  ...
  const save = async () => {
    if (!valid()) { toast('Please fill all fields correctly', 'error'); return; }
    setSaving(true);
    try {
      ...
      cancel();
    } catch (e) { toast(e instanceof Error ? e.message : 'Could not save address', 'error'); }
    finally { setSaving(false); }
  };

  const remove = async (id: string) => {
    try { setAddresses(await profileApi.deleteAddress(id)); toast('Address removed'); }
    catch (e) { toast(e instanceof Error ? e.message : 'Could not remove address', 'error'); }
  };
  const makeDefault = async (id: string) => {
    try { setAddresses(await profileApi.setDefaultAddress(id)); toast('Default address updated'); }
    catch (e) { toast(e instanceof Error ? e.message : 'Could not update default address', 'error'); }
  };
  ...
  return (
    <div className="mr-account-panel">
      ...
      {showForm && (
        <div className="mr-form-grid" style={{ marginBottom: 24 }}>
          <Field label="Label"><select value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value as AddressFormState['label'] })}><option value="home">Home</option><option value="work">Work</option><option value="other">Other</option></select></Field>
      ...
      {loading ? <p className="mr-cart-summary-tax">Loading addresses…</p> : addresses.length === 0 && !showForm ? (
        <EmptyState glyph="📍" title="No addresses saved" text="Add an address for faster checkout." />
      ) : (
        <div className="mr-addr-list">
          {addresses.map((a) => (
            <div className="mr-addr-card" key={a.id}>
...
function ProfileTab() {
  ...
  if (isInitializing) return <div className="mr-account-panel"><h2 className="mr-account-h2">Profile</h2><p className="mr-cart-summary-tax">Loading…</p></div>;
  if (!isLoggedIn) {
    return (
      <div className="mr-account-panel">
  ...
  const saveProfile = async () => {
    ...
    } catch (e) { toast(e instanceof Error ? e.message : 'Could not update profile', 'error'); }
  ...
  const saveAvatar = async () => {
    ...
    try {
      ...
      toast('Photo updated');
    } catch (e) { toast(e instanceof Error ? e.message : 'Could not update photo', 'error'); }
    finally { setSavingAvatar(false); }
  };

  const savePassword = async () => {
    if (!pw.current || pw.next.length < 8) { toast('Enter your current password and a new password (min 8 characters)', 'error'); return; }
    ...
    } catch (e) { toast(e instanceof Error ? e.message : 'Could not change password', 'error'); }
...
function Field({ label, children, full }: { label: string; children: React.ReactNode; full?: boolean }) {
  return (
    <div className={`mr-field ${full ? 'mr-field-full' : ''}`}>
