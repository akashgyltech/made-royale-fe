...
export default function ProfileNavs() {
    return (<div className="nav nav-tabs tp-tab-menu flex-column" id="profile-tab" role="tablist">
      {nav_data.map((item, i) => (<button key={i} className={`nav-link ${i === 0 ? 'active' : ''}`} id={`nav-${i + 1}-tab`} data-bs-toggle="tab" data-bs-target={`#nav-${i + 1}`} type="button" role="tab" aria-controls={`nav-${i + 1}`} aria-selected={i === 0 ? 'true' : 'false'} tabIndex={-1}>
        <span>
